 ...
 * @package Runner
 */
class BotsingRunner extends AbstractRunner
{

    private const DOCKER_CONTAINER_NAME            = 'docker-botsing-%s-%s-frame-%s';
    ...
    private const DOCKER_VOL_BOTSING_TEST_CASE     = '-v ' . __SCRIPT_DIR__ . '/../data/problems/%s-%s/frame-%s/red-botsing:/opt/runner/results';

    const QUERY_ON_FINISH       = 'UPDATE ths_problems SET docker_botsing_runner = 1 WHERE ths_problems.id = ?';
    const QUERY_SELECT_PROBLEMS = 'SELECT ths_problems.*, ths_software.project_id, ths_software.bug_id FROM ths_problems INNER JOIN ths_software ON ths_problems.software_id = ths_software.id WHERE docker_botsing_runner = 0';
    ...
    private const SETTING_TEST_CASES           = 'data.test-cases.failing.mult_botsing';
    ...
    protected function getContainerOptions(Problem $problem): array
    {
        ...
        $options[] = sprintf(self::DOCKER_ENV_MULTIPLE, 'false');
